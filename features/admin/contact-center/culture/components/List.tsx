...
import { CultureItemsList } from './ItemsList';
...
interface ListProps {
  data: Culture[];
  isLoading: boolean;
  onEdit: (item: Culture) => void;
  onDelete: (item: Culture) => void;
  searchValue: string;
          ...
          {!searchValue && (
            <>
              <div className="text-center space-y-3 z-10">
                ...
                <p className="text-slate-500 dark:text-slate-400 max-w-[320px] font-medium">
                  You haven't created any culture yet.
                </p>
              </div>
            ...
            >
              ...
              {expandedId === item.id && (
                <div className="bg-slate-50/30 dark:bg-slate-950/30 border-t border-slate-100 dark:border-slate-800">
                  <CultureItemsList cultureId={item.id} />
                </div>
              )}
            </div>
          ))}
