...
import { useAttractionItems } from '@/features/admin/content-manager/attractions/hooks/useAttractionItems';
...
import AttractionItemModal from './ItemModal';
import { AttractionItemCard } from './ItemCard';
import { DeleteConfirmDialog } from '@/components/dialogs/DeleteConfirmDialog';

export function AttractionItemsList({
  attractionId,
}: {
  attractionId: string;
}) {
  const {
    ...
    isUpdating,
    isDeleting,
  } = useAttractionItems(attractionId);
  ...
  const handleOpenEdit = (item: AttractionItem) => {
    setModal({ open: true, item });
  };

  const handleOpenDelete = (item: AttractionItem) => {
  ...
  const handleDelete = async (): Promise<boolean> => {
    let success = false;
    if (deleteModal.item) {
      success = await deleteAttractionItem(deleteModal.item.id);
    }
  ...
  const handleFinalSubmit = async (formData: FormData): Promise<boolean> => {
    let success = false;
    if (modal.item) {
      success = await updateAttractionItem(modal.item.id, formData);
    } else {
      success = await createAttractionItem(formData);
    }
        ...
        <div className="bg-slate-50 dark:bg-slate-800 space-y-4 pb-6 pt-2">
          ...
          <div className="flex items-center justify-between px-6">
            <div className="flex items-center gap-3">
              ...
              <Badge variant="secondary">{attractionItems.length}</Badge>
            </div>
            <ActionButton
              text="New Item"
            ...
            <div className="py-16 px-4 flex flex-col items-center justify-center">
              ...
              <div className="mt-6 text-center max-w-[240px]">
                <p className="text-xs leading-relaxed text-slate-400 dark:text-slate-500 font-medium italic">
                  No sub-items found. Start building your attraction category by
                  adding new records.
                </p>
              </div>
