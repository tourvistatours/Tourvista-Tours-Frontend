...
import { SlidersHorizontal, Map, MapPin, MapPinned } from 'lucide-react';
...
import { ShowcasesStats } from '../types/showcases.type';

interface Props {
  stats: ShowcasesStats | null;
  isLoading: boolean;
  onClick: () => void;
}

export default function ShowcaseHeader({ stats, isLoading, onClick }: Props) {
  ...
  return (
    <header className="pb-2 space-y-6">
      ...
      <div className="flex justify-center md:justify-start">
        <AdminBadge label="Showcases" variant="blue" />
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center md:items-end justify-between gap-8">
        <div className="flex-1 w-full">
          <PageHeader
            title="Showcases"
            highlight="View"
            subtitle="Manage showcase categories and control how they appear to users on the website."
            icon={MapPinned}
          />
        </div>
