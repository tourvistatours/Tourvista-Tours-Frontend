...
import { ShowcaseItemsList } from './ItemsList';
...
interface ListProps {
  data: Showcase[];
  isLoading: boolean;
  onEdit: (item: Showcase) => void;
  onDelete: (item: Showcase) => void;
  searchValue: string;
          ...
          {!searchValue && (
            <>
              <div className="text-center space-y-3 z-10">
                ...
                <p className="text-slate-500 dark:text-slate-400 max-w-[320px] font-medium">
                  You haven't created any showcases yet.
                </p>
              </div>
            ...
            >
              ...
              {expandedId === item.id && (
                <div className="bg-slate-50/30 dark:bg-slate-950/30 border-t border-slate-100 dark:border-slate-800">
                  <ShowcaseItemsList showcaseId={item.id} />
                </div>
              )}
            </div>
          ))}
