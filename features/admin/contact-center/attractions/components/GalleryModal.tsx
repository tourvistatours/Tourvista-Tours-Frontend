...
export default function AttractionItemGalleryModal({
  itemId,
  title,
  isOpen,
  onClose,
  ...
  const { createAttractionItemGallery, isCreating } =
  ...
  const handleUpload = async () => {
    ...
    const success = await createAttractionItemGallery(formData);
    if (success) {
      setFiles([]);
      onClose();
    }
