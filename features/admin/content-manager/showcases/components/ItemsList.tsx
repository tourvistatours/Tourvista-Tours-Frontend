...
import { useShowcaseItems } from '@/features/admin/content-manager/showcases/hooks/useShowcaseItems';
...
import ShowcaseItemModal from './ItemModal';
import { ShowcaseItemCard } from './ItemCard';
import { DeleteConfirmDialog } from '@/components/dialogs/DeleteConfirmDialog';
import { boolean } from 'zod';

export function ShowcaseItemsList({ showcaseId }: { showcaseId: string }) {
  const {
    showcaseItems,
    createShowcaseItem,
    updateShowcaseItem,
  ...
  const handleOpenDelete = (item: ShowcaseItem) => {
  ...
  const handleDelete = async (): Promise<boolean> => {
    let success = false;
    if (deleteModal.item) {
      success = await deleteShowcaseItem(deleteModal.item.id);
    }
  ...
  const handleFinalSubmit = async (formData: FormData): Promise<boolean> => {
    let success = false;
    if (modal.item) {
      success = await updateShowcaseItem(modal.item.id, formData);
    } else {
      success = await createShowcaseItem(formData);
    }
        ...
        <div className="bg-slate-50 dark:bg-slate-800 space-y-4 pb-6 pt-2">
          ...
          <div className="flex items-center justify-between px-6">
            <div className="flex items-center gap-3">
              ...
              <Badge variant="secondary">{showcaseItems.length}</Badge>
            </div>
            <ActionButton
              text="New Item"
            ...
            <div className="py-16 px-4 flex flex-col items-center justify-center">
              ...
              <div className="mt-6 text-center max-w-[240px]">
                <p className="text-xs leading-relaxed text-slate-400 dark:text-slate-500 font-medium italic">
                  No sub-items found. Start building your showcase category by
                  adding new records.
                </p>
              </div>
