    ...
    <section className="rounded-[2.5rem] p-6 md:p-8 mb-8 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl shadow-slate-200/20 dark:shadow-none transition-all">
      ...
      <div className="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 rounded-2xl bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-600 dark:text-amber-400 border border-amber-100/50 dark:border-amber-500/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <Tooltip delayDuration={300}>
                  ...
                  <TooltipContent className="max-w-xs rounded-xl p-3 bg-slate-900 text-white border-none shadow-2xl">
                    <p className="text-[11px] font-medium leading-relaxed">
                      Find specific reviews by{' '}
                      <span className="text-blue-400 font-bold">
                        rating, comments, tour{' '}
                      </span>
                      (tourId or titles){' '}
                      <span className="text-blue-400 font-bold">
                        , or user details{' '}
        ...
        {/* VISIBILITY STATUS */}
        <FilterGroup label="Public Status" className="md:col-span-2">
          <Select
            value={filters.isVisible?.toString() || 'all'}
            onValueChange={(val) =>
              onChange('isVisible', val === 'all' ? undefined : val)
            }
          >
        ...
        {/* FEATURED STATUS */}
        <FilterGroup label="Highlight" className="md:col-span-2">
          <Select
            value={filters.isFeatured?.toString() || 'all'}
            onValueChange={(val) =>
              onChange('isFeatured', val === 'all' ? undefined : val)
            }
          >
            <SelectTrigger className="!h-12 w-full cursor-pointer rounded-2xl bg-slate-50/50 dark:bg-slate-800/40 border-slate-100 dark:border-slate-700/50 text-[13px] font-bold ring-offset-transparent focus:ring-1 focus:ring-blue-500/20">
              <SelectValue placeholder="All" />
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          <CalendarIcon className="mr-2.5 h-4 w-4 text-amber-500 shrink-0 opacity-70" />
